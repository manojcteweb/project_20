...
class TestCreditCheckService:
    ...
    @patch('notification_service.NotificationService.send_credit_check_result')
    def test_complete_credit_check(self, mock_send_notification, credit_check_service, mock_db):
        user_id = "user123"
        result = "approved"
        ...
        with patch('requests.post') as mock_post:
            ...
            # Call the method under test
            credit_check_service.complete_credit_check(user_id, result)

            # Verify that the database update was called
            mock_db.credit_checks.update_one.assert_called_once_with(
                {"user_id": user_id},
                {"$set": {"status": "completed", "result": result}}
            )

            # Verify that the notification service was called
            mock_send_notification.assert_called_once_with(user_id, result)
